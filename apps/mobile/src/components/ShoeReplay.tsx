import { betLabel } from "@ba-predict/engine";
import { Text, View } from "react-native";
import { formatPercent, formatUnits } from "../lib/format";
import { useShoeReplay } from "../state/store";
import { usePalette } from "../theme";
import { Card, Hint, Notice, Prose, useStyles } from "./ui";

/**
 * What every staking plan would have done on the shoe you actually played.
 *
 * Hindsight on real coups — but on ONE shoe, which is why the cost column
 * matters more than the ranking.
 */
/**
 * Net as a share of everything the plan staked.
 *
 * Signed, under a direction-neutral header ("per unit") on purpose: this is
 * the house edge with the sign flipped, and putting a negative number under a
 * word like "cost" is the exact mislabelling this app has already had to fix
 * three times.
 */
function perUnit(row: { netUnits: number; totalStakedUnits: number }): number {
  return row.totalStakedUnits === 0 ? 0 : row.netUnits / row.totalStakedUnits;
}

export default function ShoeReplay() {
  const shoe = useShoeReplay();
  const p = usePalette();
  const s = useStyles(p);

  if (!shoe) {
    return (
      <Card title="This shoe, replayed" subtitle="Available once you have recorded some results">
        <Prose>
          Record a few coups on the Table tab and every staking plan is replayed against them
          here — the same outcomes, ten different ways of sizing the bet.
        </Prose>
      </Card>
    );
  }

  const { replay, finished } = shoe;

  return (
    <Card
      title={finished ? "Last shoe, replayed" : "This shoe, replayed"}
      subtitle={`${replay.coups} coups, every plan betting ${betLabel(replay.bet)}`}
    >
      <View style={{ gap: 6 }}>
        <View style={{ flexDirection: "row" }}>
          <Text style={[s.fieldLabel, { flex: 1 }]}>PLAN</Text>
          <Text style={[s.fieldLabel, { width: 58, textAlign: "right" }]}>NET</Text>
          <Text style={[s.fieldLabel, { width: 52, textAlign: "right" }]}>PEAK</Text>
          <Text style={[s.fieldLabel, { width: 58, textAlign: "right" }]}>PER UNIT</Text>
        </View>
        {replay.rows.map((row) => (
          <View key={row.progression} style={{ flexDirection: "row", paddingVertical: 3 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: p.text, fontSize: 13, fontWeight: "600" }}>{row.name}</Text>
              {row.brokeDownAt !== null ? (
                <Text style={{ color: p.warn, fontSize: 10 }}>
                  {row.brokeDownReason === "table-limit" ? "hit table max" : "went broke"} at coup{" "}
                  {row.brokeDownAt + 1}
                </Text>
              ) : null}
            </View>
            <Text
              style={{
                width: 58,
                textAlign: "right",
                fontSize: 13,
                color: row.netUnits >= 0 ? p.accent : p.danger,
              }}
            >
              {row.netUnits >= 0 ? "+" : "−"}
              {formatUnits(Math.abs(row.netUnits))}u
            </Text>
            <Text
              style={{
                width: 52,
                textAlign: "right",
                fontSize: 13,
                color: row.peakStakeUnits > 8 ? p.danger : p.text,
              }}
            >
              {formatUnits(row.peakStakeUnits)}u
            </Text>
            <Text
              style={{
                width: 58,
                textAlign: "right",
                fontSize: 13,
                color: perUnit(row) >= 0 ? p.accent : p.danger,
              }}
            >
              {perUnit(row) >= 0 ? "+" : "−"}
              {formatPercent(Math.abs(perUnit(row)), 1)}
            </Text>
          </View>
        ))}
      </View>

      {replay.approximate ? (
        <Notice tone="warn">
          Your table pays a reduced rate on a Banker win with 6, and the recorded coups do not
          say which wins were on 6 — the Banker figures here are slightly generous.
        </Notice>
      ) : null}

      <Hint>
        One shoe of hindsight. Whichever plan is on top won that much variance and nothing
        more — re-run this on the next shoe and the order will move. Per unit is the column
        worth reading: net over everything that plan staked. However wildly they differ here,
        over a long enough run every row settles near −1.06% on Banker, which is the thing no
        staking plan changes.
      </Hint>
    </Card>
  );
}
